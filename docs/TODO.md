...
- [easy] Phase 7 re-smoke against the renamed APK: `adb install -r` the `ca.toadlybroodledev.minimalist.dev` build (the prior `ca.toadlybroodledev.sublist` install becomes orphaned since the applicationId changed), confirm launcher + widget + MainActivity restore-state still bind without crashing under the new package. Reason: Phase 10.2 changed install identity; the Phase 7 smoke test was against the old applicationId.
- [easy] Phase 10.2 follow-up: delete the Phase 10.4 carryover `ProfileFragment.java` stub now that the package rename invalidates the legacy `ca.toadlybroodledev.sublist`-saved FragmentManager state — there is no path left for the old saved state to instantiate the class under the new package. Also delete the related `AppSettings.m4935c` / `m4946k` legacy collection-path methods flagged by the Phase 10 follow-up cycle. Reason: 10.4 review note + Phase-10 follow-up note both deferred cleanup to 10.2 closeout.
