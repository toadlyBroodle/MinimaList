...
- [medium] 4.2 Port `model/` (formerly `p032c`) — 1 class survives: `OutlineRow` (was `p032c.C0554a`). `C0556c` (cloud user-profile) is Phase-4 deletion per RENAME-MAP. Reason: SPEC Phase 4; smallest port; unblocks 4.3.
- [medium] 4.3 Port `iface/` (formerly `p031b`) — 4 callbacks: `HostContract`, `RowActionListener`, `FragmentHost`, `OutlineHost`. Drop `mo4756C`/`mo4758E`/`mo4778p` from HostContract during the port. Reason: SPEC Phase 4; unblocks the bulk root-package port.
- [hard] 4.4 Port the renamed root-package classes (`AppSettings`, `OutlineStore`, `OutlineTree`, `OutlineRowView`, `SublistFragment`, `Clipboard`, `DateTimeUtil`, `SublistRenderer`, `IndentManager`, `StringArraySpinnerAdapter`, `DrawerToggle`, `NewSublistDialog`, `WelcomeSublistFragment`). Reason: SPEC Phase 4.
