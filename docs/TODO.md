...
- [medium] 2.1 Read every decompiled class and produce `docs/RENAME-MAP.md` covering all remaining `C05NN` / `ViewOnClickListenerC05NN` / `InterfaceC05NN` (Phase 3 deletions shrink the scope substantially — 4 classes + p030a are gone). Reason: SPEC Phase 2; blocks every Phase 4 port item.
- [easy] 1.1 Copy decompiled res/ into app/src/main/res/ with locale-dir whitelist; cherry-pick a small subset to keep. Reason: SPEC Phase 1; needed for Phase 4 R.* references to resolve.
- [easy] 1.2 Rewrite AndroidManifest.xml from scratch declaring only MainActivity + WidgetProvider + ReceiverNotification; drop every cloud permission / component catalogued in `REMOVED-CLOUD-SURFACE.md` Phase 1 carryover. Reason: SPEC Phase 1.
- [easy] 1.4 `./gradlew :app:assembleDebug` green after Phase 1 resource + manifest port. Reason: SPEC Phase 1; gates Phase 4 starts.
