...
- [medium] 4.3 Port `iface/` (formerly `p031b`) — 4 callbacks: `HostContract`, `RowActionListener`, `FragmentHost`, `OutlineHost`. Drop `mo4756C`/`mo4758E`/`mo4778p` from HostContract during the port. Note: interfaces reference root-package types (ActMain, SublistFragment, OutlineTree, etc.) not yet in the active tree; compilation requires 4.4. Reason: SPEC Phase 4; unblocks the bulk root-package port.
