...
- [ ] [easy] [should-fix] `MainActivity.java:127` — `mo4759F()` builds a notification using `getPackageName()` as the channel ID, which is not a registered channel; notification silently dropped on API 26+ (all modern devices). SPEC 6.2 names only `ReceiverNotification` — this path will remain broken after Phase 6.2. Proposed fix: expand SPEC 6.2 text to include `MainActivity.mo4759F()` as a second path needing channel registration, and pass the same channel ID registered for `ReceiverNotification`.
