...
- [ ] [easy] [should-fix] `MainActivity.java:533` — `onRequestPermissionsResult` override omits `super.onRequestPermissionsResult()`, unlike every other lifecycle override in the file; `FragmentManager` cannot dispatch permission results to fragments, so any future fragment permission request (e.g. `POST_NOTIFICATIONS` for Phase 8.2 reminders) silently receives no result. Proposed fix: add `super.onRequestPermissionsResult(i, strArr, iArr)` as the first line of the method.
- [ ] [easy] [should-fix] `OutlineFab.java:27` — `m4800a(true, true)` sets `setVisibility(GONE)` instead of showing the FAB; the "animated show" branch silently hides the view, inverting its contract. The `(false, true)` / `(true, false)` / `(false, false)` call sites are correct; only `(true, true)` is wrong. Proposed fix: replace `setVisibility(GONE)` in the `visible && animated` branch with `show()`, or `setVisibility(VISIBLE)` if animation is intentionally dropped.
