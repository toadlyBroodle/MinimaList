 ...
 *
 * (c) SublistFragment.onAttach did not initialize the static f3901c host reference.
 *     Clicking a row in WelcomeSublistFragment before the async m4755B callback fired
...
public class Phase7SmokeTest {
    ...
    // ── (c) SublistFragment.onAttach initializes f3901c before interaction fires ────

    @Test
    public void sublistFragmentOnAttachInitializesHostContract() throws Exception {
        String src = readFile(
                "src/main/java/ca/toadlybroodledev/sublist/SublistFragment.java");
        // onAttach must assign f3901c so that WelcomeSublistFragment rows can be
        // safely clicked before m4755B's async callback fires.
        int onAttachIdx = src.indexOf("public void onAttach(Context context)");
        assertTrue("SublistFragment must have onAttach(Context)", onAttachIdx >= 0);
        String onAttachBody = src.substring(onAttachIdx,
                src.indexOf("}", onAttachIdx) + 1);
        assertTrue("SublistFragment.onAttach must assign f3901c = (HostContract) context",
