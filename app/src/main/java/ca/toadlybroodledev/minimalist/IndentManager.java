...
public class IndentManager {
    ...
    private void m4980a(OutlineFragment fragment, ArrayList<OutlineRowView> rows) {
        int start;
        int end;
        int i = 0;
        if (OutlineFragment.m4893aj() == -1) {
            ...
            start = OutlineFragment.m4893aj();
            end = OutlineFragment.m4894ak() + 1;
        }
        int size = rows.size();
        if (end >= size || start >= size) {
            OutlineFragment.m4897b(-1, -1);
            i = 0;
            end = rows.size();
        } else {
            i = start;
    ...
    private void m4981b(OutlineFragment fragment, OutlineRowView rv) {
    ...
    private void m4982c(OutlineFragment fragment, OutlineRowView rv) {
